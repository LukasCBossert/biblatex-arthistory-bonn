...
Note that \cs{smartcite} and \cs{autocite} behave a little bit differently than in \enquote{standard} |bib|\LaTeX{} styles. When appearing in a footnote, both commands will behave as |arthistory-bonn|'s \cs{cite} rather than \cs{parencite}. In addition to that, by default \cs{autocite} appearing in the text body behaves like \cs{footcite}.
